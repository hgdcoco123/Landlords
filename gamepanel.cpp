    ...
    QPoint roleImgPos[] =
        {
            ...
            QPoint(cardsRect[2].right() - 10, cardsRect[2].top() - 10),   //左侧机器人
        };
    ...
    QPoint pos[] =
        {
            ...
            QPoint(m_baseCardPos.x(), m_baseCardPos.y() + curPos * unit[2])  //用户玩家
        };
...
void GamePanel::updatePlayerCards(Player *player)
{
    ...
    for(int i = 0; i < list.size(); ++i)
    {
        ...
        if(m_contextMap[player].align == Horizontal)
        {
            ...
            //m_cardsRect = QRect(leftX, topY,  m_cardSize.width(), m_cardSize.height());
...
{
    // ========== 新增：仅在叫地主阶段显示分数 ==========
    if(m_gameStatus != GameControl::CallingLord)
    {
        return; // 非叫地主阶段，直接返回，不显示分数
    }
    //显示抢地主提示信息
