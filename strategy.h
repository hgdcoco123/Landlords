...
public:
    ...
    //4.能大过指定牌时，判断是出牌还是放行，返回true->出牌，返回false->放行
    ...
    //8.按牌型找牌，并且指定要找的牌是否大过指定的牌型
    QVector<Cards> findCardType(PlayHand hand, bool beat);

    //9.从指定的Cards对象中挑选出满足条件的顺子
    void pickSeqSingle(QVector<QVector<Cards>> &allSeqRecord, QVector<Cards> &seqSingle, Cards& cards);
    //10.最优的顺子的集合的筛选函数
