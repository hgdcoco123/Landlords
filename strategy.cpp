...
Cards Strategy::getGreaterCards(PlayHand type)
{
    ...
    remain.remove(Strategy(m_player, remain).pickOptimalSeqSingle());
...
void Strategy::pickSeqSingle(QVector<QVector<Cards> > &allSeqRecord, QVector<Cards> &seqSingle, Cards &cards)
{
    if (!m_player ) {
        qCritical() << "[Strategy] pickSeqSingles：Player失效，终止递归！";
        return;
    }
    //1.得到所有顺子的组合
    ...
    else //2.对顺子进行筛选
    {
        ...
        for(int i = 0; i < allSeq.size(); ++i)
        {
            ...
            pickSeqSingle(allSeqRecord, seqArray, temp);
        }
    }
}
...
QVector<Cards> Strategy::pickOptimalSeqSingle()
{
    ...
    //Strategy(m_player, m_cards).pickSeqSingle(seqRecord, seqSingles, m_cards);
...
QVector<Cards> Strategy::getSeqPairOrSeqSingle(CardInfo &info)
{
    ...
    else
    {
        for(Card::CardPoint point = info.begin; point <= info.end; point = (Card::CardPoint)(point + 1))
        {
            ...
            Cards alreadyFollowedCards; //存储后续找到的满足条件的1连对
